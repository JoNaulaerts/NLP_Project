...
Enhanced with provider selection, refined design, and professional styling
"""
...
# Professional styling with refined colors
st.markdown("""
<style>
...
def render_sidebar():
    with st.sidebar:
        ...
        st.metric("PDFs Uploaded", len(st.session_state.uploaded_docs))
...
def render_upload_page():
    ...
    st.markdown(
        '<p class="muted">Upload documents to index them into ChromaDB for RAG.</p>',
        unsafe_allow_html=True
    )
    ...
    with left:
        st.markdown(
            '<div class="card-strong"><h3>Upload Documents</h3></div>',
            unsafe_allow_html=True
        )
        
        # Multi-format file uploader (NOW WITH PPTX!)
        files = st.file_uploader(
            "Choose files to index",
            type=["pdf", "txt", "md", "docx", "py", "csv", "pptx"],  # ← ADDED PPTX
            accept_multiple_files=True,
            help="Supported: PDF, TXT, Markdown, Word, Python, CSV, PowerPoint"
        )
        
        if files:
            st.caption(
                f"📄 {len(files)} file(s) selected. "
                "Files are saved to `.data/uploaded_docs` and indexed into ChromaDB."
            )
            ...
            if st.button(
                ...
                for uf in files:
                    ...
                    with st.spinner(f"Indexing {uf.name}..."):
                        from src.ml_learning_assistant.tools.upload_to_chromadb import (
                            upload_document_to_chromadb
                        )
                    ...
                    if res.get("success"):
                        ...
                        st.success(
                            f"✅ {uf.name} | "
                            f"slides/pages/rows: {res.get('pages')} | "
                ...
                st.info(f"📊 Indexed {ok}/{len(files)} files | chunks added: {total_chunks}")
        
        st.markdown("---")
        st.markdown(
            '<div class="card"><h3>📚 Uploaded files tracked</h3></div>',
            unsafe_allow_html=True
        )
        
        if st.session_state.uploaded_docs:
            for name in sorted(st.session_state.uploaded_docs):
                # Show file type icon (NOW WITH SLIDES!)
                ...
                icon = {
                    ...
                    ".py": "🐍",
                    ".csv": "📊",
                    ".pptx": "🎠"  # ← NEW: PowerPoint icon
                }.get(ext, "📄")
                st.write(f"{icon} {name}")
        else:
            st.write("No uploaded files tracked yet.")
        
        if st.button("🗑️ Clear uploaded list (UI only)", use_container_width=True):
    ...
    with right:
        st.markdown(
            '<div class="card-strong"><h3>ChromaDB status</h3></div>',
            unsafe_allow_html=True
        )
...
def render_quiz_page():
    ...
    with col1:
        ...
        if st.session_state.quiz_obj:
            ...
            if st.session_state.quiz_submitted:
                ...
                st.markdown('<div class="glass-card-strong">', unsafe_allow_html=True)
                st.markdown("### 🎯 Results")
                st.metric("Score", f"{score}/{n} ({percentage:.0f}%)")
    ...
    with col2:
        ...
        - 📚 **Upload First**: Index relevant PDFs for accurate questions
...
def render_stats_page():
    ...
    with col_a:
        ...
        config_data = {
            ...
            "Storage Dir": os.getenv("CREWAI_STORAGE_DIR", "N/A"),
        }
